# Microservice Api Rachid
Microservice para consumir e buscar informações de Pokemons.

# Executando Localmente
Para executar localmente, será necessário algumas configurações de ambiente.
Antes de executar, é recomentado realizar um `clean` e `install` no maven, para baixar todas as dependências do projeto.

#Postman
* Para testar as requisições de API Rest, baixar acessando a URL: https://www.postman.com/downloads/

#Banco de Dados
* Deve-se alterar também o acesso ao banco, hoje está sendo utilizado postgres, então se necessário, poderá substituir
* o banco utilizado, mas deverá se atentar as credenciais de acesso e dependências (drivers).

#JAVA 17
* Baixar o JAVA 17 acessando a URL: https://download.oracle.com/java/17/latest/jdk-17_windows-x64_bin.zip
* Após o download, acessar o disco `C`, entrar na pasta `Arquivos de programas` e na pasta `JAVA`, colar o java 17 lá.
* Na IDE, configurar para que o mesmo utilize o java 17, apontando para o caminho da pasta(passo anterior).

#Kotlin
* Instalar o plugin do kotlin pelo Intellij no caminho: `File -> Settings -> Plugins -> Marketplacet e buscar por kotlin` e em seguida reiniciar a IDE.
* Ou acessar o plugin direto pela URL: https://plugins.jetbrains.com/plugin/6954-kotlin
* Verificar se o SDK do kotlin foi adicionado no Intellij: `File -> Project structure -> SDKs`

#Swagger
* Está instalado já o Swagger, e quando rodar o projeto, poderá acessar a documentação da API através da URL: http://localhost:8080/swagger-ui/index.html
